...
class Oracle_benchmark(Simulator):
    def __init__(self, learning_prefix = "") -> None: 
        ...
        self.identifier = 'almightyoracle'
    ...
    def flow_to_dataframe(self, f):
        ...
        s = [self.focs.instance.I_a] + [self.focs.instance.intervals_start] + [self.breaks[:-1]] + [self.len_i] + [[f['i'+str(i)]['t']/self.focs.instance.tau /self.focs.instance.len_i[i] for i in self.focs.instance.I_a]] + [[0]*len(self.focs.instance.I_a) for j in self.focs.instance.jobs]
        for jid, j in enumerate(self.focs.instance.jobs):
            for i in self.focs.instance.J_inverse['j'+str(j)]:
                s[5+jid][i] = f['j'+str(j)]['i'+str(i)] / self.focs.instance.tau /self.focs.instance.len_i[i]
        s = pd.DataFrame(s).T
        s.columns = ['time','breakpoints','breaks','len_i','agg'] + ['j'+str(j) for j in self.focs.instance.jobs]
        return s
    ...
    def step(self, curr_time: time, df_agg_timeseries: pd.DataFrame, df_usr_sessions: pd.DataFrame, active_session_info: pd.DataFrame) -> None:
        ...
        if curr_time != dt.time(hour=23, minute=45):
            ...
            schedule.loc[len(schedule)] = [len(schedule), schedule['breakpoints'].iloc[-1]+1, self.breaks[-1] , 1, 0] + [0 for j in self.focs.instance.jobs]
            self.len_i += [1]
    ...
    def publish_results(self, output_dir: str, prefix: Optional[str] = None) -> None:
        ...
        logger.debug('what does the almighty oracly say? l;ajsdf;llkdkjfjkdkdsfkkdkdkdkdkdkdkdkdkdkdkdkdk')
