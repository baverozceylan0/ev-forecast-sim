...
# Baver's code
from src.common.session_generator import generate_sessions_from_profile, generate_sessions_naive
...
class lLYNCS_naive(Simulator):
    ...
    def preprocessing(self,input, upcoming, curr_time=None):
        ...
        # input['total_energy_Wh'] = [max(0,input['total_energy'].iloc[x]-self.supplied_energy[str(input['session_id'].iloc[x])])*1000 for x in range(0,len(input))] 
    ...
    def focs_scheduler(self,input, upcoming, curr_time, lyncs_mode = 'linear'):
        ...
        self.instance = FOCSinstance(input, self.timeStep, periodicity=True)
        flowNet = FlowNet()
    ...
    def step(self, curr_time: time, df_agg_timeseries: pd.DataFrame, df_usr_sessions: pd.DataFrame, active_session_info: pd.DataFrame) -> None:
        ...
        input = generate_sessions_naive(input)
    ...
    def supplied_by_milestone(self,id,milestone_t, taus):
        ...
        if milestone_t not in self.breaks:
            logger.info('Milestone_t = {} not in breakpoints. Adding breakpoint and updating sim_profiles.'.format(milestone_t))
    ...
    def publish_results(self, output_dir: str, prefix: Optional[str] = None) -> None:
        ...
        logger.debug('what does the naive lynx say? l;ajsdf;llkdkjfjkdkdsfkkdkdkdkdkdkdkdkdkdkdkdkdk')
