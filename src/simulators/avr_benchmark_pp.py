# perfect prediction scenario with AVR.
...
from src.common.FlowbasedOfflineChargingScheduler.AVR import AVR
...
class AVR_benchmark(Simulator):
    def __init__(self, learning_prefix = "") -> None: 
        ...
        self.identifier = 'avr_benchmark_pp'
    ...
    def flow_to_dataframe(self, f):
        ...
        s = [self.avr.instance.I_a] + [self.avr.instance.intervals_start] + [self.breaks[:-1]] + [self.len_i] + [[f['i'+str(i)]['t']/self.avr.instance.tau /self.avr.instance.len_i[i] for i in self.avr.instance.I_a]] + [[0]*len(self.avr.instance.I_a) for j in self.avr.instance.jobs]
        for jid, j in enumerate(self.avr.instance.jobs):
            for i in self.avr.instance.J_inverse['j'+str(j)]:
                s[5+jid][i] = f['j'+str(j)]['i'+str(i)] / self.avr.instance.tau /self.avr.instance.len_i[i]
        s = pd.DataFrame(s).T
    ...
    def qos_2(self, data): # qos_2 in Danner and de Meer (2021)
        # NOTE trivially 1 in this setup, as upon arrival, we charge EVs at their average required power
