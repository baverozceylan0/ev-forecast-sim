...
class AggModelEvaluationPipeline(FlowSpec):
    ...
    def model_testing(self):
        ...
        logger.debug(f"Real EV shape: {real_ev.shape} | Real Energy shape: {real_energy.shape}")
        logger.debug(f"Forecast EV shape: {forecast_ev.shape} | Forecast Energy shape: {forecast_energy.shape}")
        ...
        np.save(os.path.join(results_folder, 'real_ev.npy'), real_ev)
        np.save(os.path.join(results_folder, 'real_energy.npy'), real_energy)
        np.save(os.path.join(results_folder, 'forecast_ev.npy'), forecast_ev)
        np.save(os.path.join(results_folder, 'forecast_energy.npy'), forecast_energy)
